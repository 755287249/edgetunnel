...
const 支持的节点协议 = ['vless', 'trojan', 'ss'];

function 规范化协议类型列表(value) {
	const source = Array.isArray(value)
		...
		: typeof value === 'string'
			...
			: [];
	const aliases = {
		vless: 'vless', vl: 'vless',
		trojan: 'trojan', tro: 'trojan',
		ss: 'ss', shadowsocks: 'ss', shadowrocket: 'ss',
	};
	const normalized = source
		.map(item => aliases[String(item || '').trim().toLowerCase()])
		.filter(Boolean);
	return [...new Set(normalized)].filter(item => 支持的节点协议.includes(item));
}

function 节点协议显示名(protocol) {
...
(() => {
  ...
  const patchConfig = (config) => {
    ...
    config.协议类型列表 = values;
    config.协议类型 = values[0];
    return config;
  };
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (method === 'POST' && /(?:^|\\/)admin\\/config\\.json(?:\\?|$)/.test(target) && typeof init.body === 'string') {
      try {
        ...
        if (error && /至少选择/.test(error.message || '')) throw error;
      }
    }
  ...
  if (typeof XMLHttpRequest !== 'undefined') {
    ...
    XMLHttpRequest.prototype.open = function(method, url, ...rest) {
      this.__edtMethod = String(method || 'GET').toUpperCase();
      this.__edtUrl = String(url || '');
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (访问路径 === 'admin/init') {// 重置配置为默认值
						...
						if (访问路径 === 'admin/config.json') { // 保存config.json配置
							try {
								...
								let 协议配置来源 = newConfig.协议类型列表;
								if (协议配置来源 == null) {
									try {
										const 已有配置 = JSON.parse(await env.KV.get('config.json') || '{}');
										协议配置来源 = 已有配置.协议类型列表 ?? newConfig.协议类型;
									} catch (_) {
										协议配置来源 = newConfig.协议类型;
									}
								}
								const 协议类型列表 = 规范化协议类型列表(协议配置来源);
								if (!协议类型列表.length) return new Response(JSON.stringify({ error: '请至少选择一种节点协议' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
								newConfig.协议类型列表 = 协议类型列表;
								newConfig.协议类型 = 协议类型列表[0]; // 保留旧版前端/配置兼容字段
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						const 配置协议类型列表 = 规范化协议类型列表(config_JSON.协议类型列表 ?? config_JSON.协议类型);
						const 协议类型列表 = (url.searchParams.has('surge') || ua.includes('surge'))
							? [...new Set(配置协议类型列表.map(item => item === 'vless' ? 'trojan' : item))]
							: 配置协议类型列表;
						let 订阅内容 = '';
						if (订阅类型 === 'mixed') {
							...
							订阅内容 = 其他节点LINK + 完整优选IP.flatMap(原始地址 => {
								// 统一正则: 匹配域名/IPv4/IPv6地址 + 可选端口 + 可选备注
								...
								return 协议类型列表.map(协议类型 => {
									let 节点端口 = 原始节点端口;
									let 当前完整节点路径 = 完整节点路径;
									const 当前节点备注 = 协议类型列表.length > 1 ? `${节点备注} [${节点协议显示名(协议类型)}]` : 节点备注;
									...
									if (协议类型 === 'ss') {
										...
										当前完整节点路径 = (当前完整节点路径.includes('?') ? 当前完整节点路径.replace('?', '?enc=' + config_JSON.SS.加密方式 + '&') : (当前完整节点路径 + '?enc=' + config_JSON.SS.加密方式)).replace(/([=,])/g, '\\$1');
										if (!isSubConverterRequest) 当前完整节点路径 += ';mux=0';
										return `${协议类型}://${btoa(config_JSON.SS.加密方式 + ':00000000-0000-4000-8000-000000000000')}@${节点地址}:${节点端口}?plugin=v2${encodeURIComponent('ray-plugin;mode=websocket;host=example.com;path=' + (config_JSON.随机路径 ? 随机路径(当前完整节点路径) : 当前完整节点路径) + (config_JSON.SS.TLS ? ';tls' : '')) + ECHLINK参数 + TLS分片参数}#${encodeURIComponent(当前节点备注)}`;
									}

									const 传输路径参数值 = 获取传输路径参数值(config_JSON, 当前完整节点路径, 作为优选订阅生成器);
									return `${协议类型}://00000000-0000-4000-8000-000000000000@${节点地址}:${节点端口}?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=example.com&fp=${config_JSON.Fingerprint}&sni=example.com&${路径字段名}=${encodeURIComponent(传输路径参数值) + TLS分片参数}&encryption=none#${encodeURIComponent(当前节点备注)}`;
								});
							}).filter(Boolean).join('\n');
						} else { // 订阅转换
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false) {
	...
	const 已保存协议类型列表 = 规范化协议类型列表(config_JSON.协议类型列表 ?? config_JSON.协议类型);
	config_JSON.协议类型列表 = 已保存协议类型列表.length ? 已保存协议类型列表 : ['vless'];
	config_JSON.协议类型 = config_JSON.协议类型列表[0]; // 旧版后台仍读取此字段
	...
	config_JSON.LINKS = config_JSON.协议类型列表.map(协议类型 => {
		const 节点备注 = config_JSON.协议类型列表.length > 1 ? `${config_JSON.优选订阅生成.SUBNAME} [${节点协议显示名(协议类型)}]` : config_JSON.优选订阅生成.SUBNAME;
		if (协议类型 === 'ss') {
			const SS路径 = (config_JSON.完整节点路径.includes('?') ? config_JSON.完整节点路径.replace('?', '?enc=' + config_JSON.SS.加密方式 + '&') : (config_JSON.完整节点路径 + '?enc=' + config_JSON.SS.加密方式)) + (config_JSON.SS.TLS ? ';tls' : '');
			return `${协议类型}://${btoa(config_JSON.SS.加密方式 + ':' + userID)}@${host}:${config_JSON.SS.TLS ? '443' : '80'}?plugin=v2${encodeURIComponent(`ray-plugin;mode=websocket;host=${host};path=${SS路径};mux=0`) + ECHLINK参数}#${encodeURIComponent(节点备注)}`;
		}
		return `${协议类型}://${userID}@${host}:443?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=${host}&fp=${config_JSON.Fingerprint}&sni=${host}&${路径字段名}=${encodeURIComponent(传输路径参数值) + TLS分片参数}&encryption=none#${encodeURIComponent(节点备注)}`;
	});
	config_JSON.LINK = config_JSON.LINKS[0];
	config_JSON.LINK_ALL = config_JSON.LINKS.join('\n');
